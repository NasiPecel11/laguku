...
class SongService {
  ...
  async editSongById(id, { title, year, genre, performer, duration, albumId }) {
    ...
    const query =
      albumId == undefined
        ? {
            text: "UPDATE song SET title = $1, year = $2, genre = $3, performer = $4, duration = $5, updated_at = $6 WHERE id = $7 RETURNING id",
            values: [title, year, genre, performer, duration, updatedAt, id],
          }
        : {
            text: "UPDATE song SET title = $1, year = $2, genre = $3, performer = $4, duration = $5, albumId = $6, updated_at = $7 WHERE id = $8 RETURNING id",
            values: [
              title,
              year,
              genre,
